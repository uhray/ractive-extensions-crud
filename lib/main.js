...
define(['ractive', 'crud'], function(ractive, crud) {
  ...
  extension = Ractive.extend({
    ...
    onrender: function(options) {
      ...
      this.on('crud-update', function(event, path, object) {
        var cfg = crud.configure(),
            list;
        ...
        list = object instanceof Array ? object : [object];

        asyncMap(list, function(d, cb) {
          var p = path;

          if (p) crud(path, d[cfg.idGetter]).update(d, cb);
          else d._crud.update(d, cb);
        }, function(e, d) {
          var n = event.node;
      ...
      this.on('crud-delete', function(event, object, path) {
        ...
        if (path) crud(path, d[cfg.idGetter]).del(fn);
        else object._crud.del(fn);
      ...
      this.on('crud-create', function(event, arr, cr) {
        (cr || arr._crud).create(obj || {}, function(e, d) {
          var n = event.node;
